...
public class BitonicSort : MonoBehaviour
{
    ...
    public void Sort(ComputeBuffer inBuffer)
    {
        ...
        for (uint level = (BITONIC_BLOCK_SIZE << 1); level <= NUM_ELEMENTS; level <<= 1)
        {
            ...
            sortCS.Dispatch(KERNEL_ID_TRANSPOSE, Mathf.CeilToInt(MATRIX_WIDTH / (float)TRANSPOSE_BLOCK_SIZE), Mathf.CeilToInt(MATRIX_HEIGHT / (float)TRANSPOSE_BLOCK_SIZE), 1);
            ...
            sortCS.Dispatch(KERNEL_ID_TRANSPOSE, Mathf.CeilToInt(MATRIX_HEIGHT / (float)TRANSPOSE_BLOCK_SIZE), Mathf.CeilToInt(MATRIX_WIDTH / (float)TRANSPOSE_BLOCK_SIZE), 1);
